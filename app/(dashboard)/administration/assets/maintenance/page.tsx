'use client';

import { useState, useEffect } from 'react';
import { supabase } from '@/lib/supabase';
import { useAuth } from '@/contexts/AuthContext';
import { logAuditEvent } from '@/lib/audit';
import { PermissionGuard, Can } from '@/components/rbac/PermissionGuard';
import PageHeader from '@/components/common/PageHeader';
import KPICard from '@/components/common/KPICard';
import DataTable, { Column } from '@/components/common/DataTable';
import ConfirmDialog from '@/components/common/ConfirmDialog';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { Label } from '@/components/ui/label';
import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogTrigger } from '@/components/ui/dialog';
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from '@/components/ui/select';
import { Textarea } from '@/components/ui/textarea';
import { Wrench, Plus, Edit, Trash2, Calendar, DollarSign, Package } from 'lucide-react';
import { DropdownMenu, DropdownMenuContent, DropdownMenuItem, DropdownMenuTrigger } from '@/components/ui/dropdown-menu';
import { Badge } from '@/components/ui/badge';
import StatusBadge from '@/components/common/StatusBadge';
import { useForm, Controller } from 'react-hook-form';
import { zodResolver } from '@hookform/resolvers/zod';
import { z } from 'zod';
import { toast } from 'sonner';

const maintenanceSchema = z.object({
  asset_type: z.enum(['furniture', 'equipment', 'vehicle']),
  asset_id: z.string().min(1, 'Required'),
  maintenance_type: z.enum(['routine', 'repair', 'replacement', 'inspection']),
  description: z.string().min(1, 'Required'),
  cost: z.string().optional(),
  scheduled_date: z.string().optional(),
  completed_date: z.string().optional(),
  performed_by: z.string().optional(),
  notes: z.string().optional(),
});
type MaintenanceForm = z.infer<typeof maintenanceSchema>;

export default function AssetMaintenancePage() {
  const { company, user: currentUser } = useAuth();
  const [maintenance, setMaintenance] = useState<any[]>([]);
  const [furniture, setFurniture] = useState<any[]>([]);
  const [equipment, setEquipment] = useState<any[]>([]);
  const [vehicles, setVehicles] = useState<any[]>([]);
  const [users, setUsers] = useState<any[]>([]);
  const [loading, setLoading] = useState(true);
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editRecord, setEditRecord] = useState<any>(null);
  const [deleteId, setDeleteId] = useState<string | null>(null);
  const [deleting, setDeleting] = useState(false);

  const { register, handleSubmit, reset, control, formState: { errors, isSubmitting } } = useForm<MaintenanceForm>({
    resolver: zodResolver(maintenanceSchema),
    defaultValues: { maintenance_type: 'routine', asset_type: 'furniture' },
  });

  const load = async () => {
    if (!company?.id) return;
    setLoading(true);

    const [maintRes, furnRes, eqRes, vehRes, userRes] = await Promise.all([
      supabase
        .from('asset_maintenance')
        .select('*, performed_by_profile(first_name, last_name)')
        .eq('company_id', company.id)
        .order('scheduled_date', { ascending: false }),
      supabase.from('furniture').select('*').eq('company_id', company.id),
      supabase.from('equipment').select('*').eq('company_id', company.id),
      supabase.from('vehicles').select('*').eq('company_id', company.id),
      supabase.from('profiles').select('*').eq('company_id', company.id),
    ]);

    setMaintenance(maintRes.data ?? []);
    setFurniture(furnRes.data ?? []);
    setEquipment(eqRes.data ?? []);
    setVehicles(vehRes.data ?? []);
    setUsers(userRes.data ?? []);
    setLoading(false);
  };

  useEffect(() => { load(); }, [company?.id]);

  const getAssetName = (record: any) => {
    if (record.asset_type === 'furniture') {
      const item = furniture.find(f => f.id === record.asset_id);
      return item ? item.name : 'Unknown';
    } else if (record.asset_type === 'equipment') {
      const item = equipment.find(e => e.id === record.asset_id);
      return item ? item.name : 'Unknown';
    } else if (record.asset_type === 'vehicle') {
      const item = vehicles.find(v => v.id === record.asset_id);
      return item ? item.vehicle_number : 'Unknown';
    }
    return 'Unknown';
  };

  const openEdit = (record: any) => {
    setEditRecord(record);
    reset({
      asset_type: record.asset_type,
      asset_id: record.asset_id,
      maintenance_type: record.maintenance_type,
      description: record.description,
      cost: record.cost?.toString() ?? '',
      scheduled_date: record.scheduled_date ?? '',
      completed_date: record.completed_date ?? '',
      performed_by: record.performed_by ?? undefined,
      notes: record.notes ?? '',
    });
    setDialogOpen(true);
  };

  const onSubmit = async (data: MaintenanceForm) => {
    if (!company?.id) return;

    const payload = {
      asset_type: data.asset_type,
      asset_id: data.asset_id,
      maintenance_type: data.maintenance_type,
      description: data.description,
      cost: data.cost ? parseFloat(data.cost) : null,
      scheduled_date: data.scheduled_date || null,
      completed_date: data.completed_date || null,
      performed_by: data.performed_by,
      notes: data.notes,
    };

    if (editRecord) {
      const { error } = await supabase
        .from('asset_maintenance')
        .update({
          ...payload,
          status: data.completed_date ? 'completed' : 'scheduled',
          updated_at: new Date().toISOString(),
        })
        .eq('id', editRecord.id);

      if (error) {
        toast.error('Failed to update maintenance record');
        return;
      }

      await logAuditEvent(company.id, currentUser?.id || '', {
        action: 'asset_maintenance_updated',
        module: 'assets',
        record_id: editRecord.id,
        new_values: { description: data.description },
      });

      toast.success('Maintenance record updated');
    } else {
      const { error } = await supabase.from('asset_maintenance').insert({
        company_id: company.id,
        ...payload,
        status: data.completed_date ? 'completed' : 'scheduled',
      });

      if (error) {
        toast.error('Failed to create maintenance record');
        return;
      }

      await logAuditEvent(company.id, currentUser?.id || '', {
        action: 'asset_maintenance_created',
        module: 'assets',
        new_values: { description: data.description },
      });

      toast.success('Maintenance record created');
    }

    reset();
    setEditRecord(null);
    setDialogOpen(false);
    load();
  };

  const handleDelete = async () => {
    if (!deleteId || !company?.id) return;
    setDeleting(true);

    const { error } = await supabase.from('asset_maintenance').delete().eq('id', deleteId);

    if (error) {
      toast.error('Failed to delete maintenance record');
    } else {
      await logAuditEvent(company.id, currentUser?.id || '', {
        action: 'asset_maintenance_deleted',
        module: 'assets',
        record_id: deleteId,
      });
      toast.success('Maintenance record deleted');
      load();
    }

    setDeleteId(null);
    setDeleting(false);
  };

  const handleComplete = async (id: string) => {
    if (!company?.id) return;

    const { error } = await supabase
      .from('asset_maintenance')
      .update({
        status: 'completed',
        completed_date: new Date().toISOString().split('T')[0],
        updated_at: new Date().toISOString(),
      })
      .eq('id', id);

    if (error) {
      toast.error('Failed to mark as complete');
    } else {
      await logAuditEvent(company.id, currentUser?.id || '', {
        action: 'asset_maintenance_completed',
        module: 'assets',
        record_id: id,
      });
      toast.success('Marked as complete');
      load();
    }
  };

  const exportCSV = () => {
    const headers = ['Asset Type', 'Asset Name', 'Type', 'Description', 'Cost', 'Scheduled Date', 'Completed Date', 'Status', 'Performed By'];
    const rows = maintenance.map(m => [
      m.asset_type,
      getAssetName(m),
      m.maintenance_type,
      m.description,
      m.cost || '',
      m.scheduled_date || '',
      m.completed_date || '',
      m.status,
      m.performed_by_profile ? `${m.performed_by_profile.first_name} ${m.performed_by_profile.last_name}` : '',
    ]);

    const csv = [headers.join(','), ...rows.map(r => r.join(','))].join('\n');
    const blob = new Blob([csv], { type: 'text/csv' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = 'asset_maintenance.csv';
    a.click();
  };

  const getAssetOptions = (type: string) => {
    if (type === 'furniture') return furniture.map(f => ({ id: f.id, name: f.name }));
    if (type === 'equipment') return equipment.map(e => ({ id: e.id, name: e.name }));
    if (type === 'vehicle') return vehicles.map(v => ({ id: v.id, name: v.vehicle_number }));
    return [];
  };

  const typeColors: Record<string, string> = {
    furniture: 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400',
    equipment: 'bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-400',
    vehicle: 'bg-orange-100 text-orange-700 dark:bg-orange-900/30 dark:text-orange-400',
  };

  const maintTypeColors: Record<string, string> = {
    routine: 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400',
    repair: 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400',
    replacement: 'bg-orange-100 text-orange-700 dark:bg-orange-900/30 dark:text-orange-400',
    inspection: 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400',
  };

  const columns: Column<any>[] = [
    {
      key: 'asset',
      header: 'Asset',
      cell: (row) => (
        <div className="flex items-center gap-3">
          <div className="p-2 rounded-lg bg-blue-50 dark:bg-blue-950/30">
            <Package className="h-4 w-4 text-blue-600 dark:text-blue-400" />
          </div>
          <div>
            <p className="font-medium text-gray-900 dark:text-white text-sm">
              {getAssetName(row)}
            </p>
            <Badge
              variant="secondary"
              className={`${typeColors[row.asset_type] || typeColors.furniture} capitalize text-xs`}
            >
              {row.asset_type}
            </Badge>
          </div>
        </div>
      ),
    },
    {
      key: 'maintenance_type',
      header: 'Type',
      sortable: true,
      cell: (row) => (
        <Badge
          variant="secondary"
          className={`${maintTypeColors[row.maintenance_type] || maintTypeColors.routine} capitalize`}
        >
          {row.maintenance_type}
        </Badge>
      ),
    },
    {
      key: 'description',
      header: 'Description',
      cell: (row) => <span className="text-sm text-gray-600 dark:text-gray-400 truncate max-w-[200px]">{row.description}</span>,
    },
    {
      key: 'cost',
      header: 'Cost',
      sortable: true,
      cell: (row) => (
        <div className="flex items-center gap-1 text-sm text-gray-600 dark:text-gray-400">
          <DollarSign className="h-3 w-3" />
          <span>{row.cost ? row.cost.toFixed(2) : '—'}</span>
        </div>
      ),
    },
    {
      key: 'scheduled_date',
      header: 'Scheduled',
      sortable: true,
      cell: (row) => (
        <div className="flex items-center gap-1 text-sm text-gray-600 dark:text-gray-400">
          <Calendar className="h-3 w-3" />
          <span>{row.scheduled_date || '—'}</span>
        </div>
      ),
    },
    {
      key: 'completed_date',
      header: 'Completed',
      sortable: true,
      cell: (row) => row.completed_date ? (
        <div className="flex items-center gap-1 text-sm text-gray-600 dark:text-gray-400">
          <Calendar className="h-3 w-3" />
          <span>{row.completed_date}</span>
        </div>
      ) : <span className="text-sm text-gray-400">—</span>,
    },
    {
      key: 'status',
      header: 'Status',
      sortable: true,
      cell: (row) => <StatusBadge status={row.status} />,
    },
    {
      key: 'actions',
      header: '',
      headerClassName: 'w-10',
      cell: (row) => (
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="ghost" size="icon" className="h-8 w-8">
              <Edit className="h-4 w-4" />
            </Button>
          </DropdownMenuTrigger>
          <DropdownMenuContent align="end">
            <DropdownMenuItem onClick={() => openEdit(row)}>
              <Edit className="h-4 w-4 mr-2" />Edit
            </DropdownMenuItem>
            {row.status === 'scheduled' && (
              <DropdownMenuItem onClick={() => handleComplete(row.id)}>
                <Wrench className="h-4 w-4 mr-2" />Mark Complete
              </DropdownMenuItem>
            )}
            <Can resource="assets.maintenance" action="delete">
              <DropdownMenuItem className="text-red-600" onClick={() => setDeleteId(row.id)}>
                <Trash2 className="h-4 w-4 mr-2" />Delete
              </DropdownMenuItem>
            </Can>
          </DropdownMenuContent>
        </DropdownMenu>
      ),
    },
  ];

  const scheduled = maintenance.filter(m => m.status === 'scheduled').length;
  const completed = maintenance.filter(m => m.status === 'completed').length;
  const totalCost = maintenance.reduce((sum, m) => sum + (m.cost || 0), 0);

  return (
    <PermissionGuard permission="assets.view" fallback={<div className="p-6 text-center text-gray-500">You don't have permission to view asset maintenance</div>}>
      <div className="space-y-6">
      <PageHeader
        title="Asset Maintenance"
        description="Track asset maintenance and repairs"
        breadcrumbs={[{ label: 'Administration' }, { label: 'Assets' }, { label: 'Maintenance' }]}
      >
        <div className="flex gap-2">
          <Button size="sm" variant="outline" onClick={exportCSV}>
            Export CSV
          </Button>
          <Can resource="assets.maintenance" action="create">
            <Dialog open={dialogOpen} onOpenChange={open => { if (!open) { setEditRecord(null); reset(); } setDialogOpen(open); }}>
              <DialogTrigger asChild>
                <Button size="sm" className="bg-blue-600 hover:bg-blue-700">
                  <Plus className="h-4 w-4 mr-2" />Add Maintenance
                </Button>
              </DialogTrigger>
              <DialogContent>
                <DialogHeader>
                  <DialogTitle>{editRecord ? 'Edit Maintenance' : 'Add Maintenance Record'}</DialogTitle>
                </DialogHeader>
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 pt-2">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <Label>Asset Type *</Label>
                      <Controller name="asset_type" control={control} render={({ field }) => (
                        <Select onValueChange={field.onChange} value={field.value}>
                          <SelectTrigger className="mt-1"><SelectValue /></SelectTrigger>
                          <SelectContent>
                            <SelectItem value="furniture">Furniture</SelectItem>
                            <SelectItem value="equipment">Equipment</SelectItem>
                            <SelectItem value="vehicle">Vehicle</SelectItem>
                          </SelectContent>
                        </Select>
                      )} />
                    </div>
                    <div>
                      <Label>Asset *</Label>
                      <Controller name="asset_id" control={control} render={({ field }) => (
                        <Select onValueChange={field.onChange} value={field.value ?? ''}>
                          <SelectTrigger className="mt-1"><SelectValue placeholder="Select asset" /></SelectTrigger>
                          <SelectContent>
                            {getAssetOptions(control._formValues.asset_type || 'furniture').map(opt => (
                              <SelectItem key={opt.id} value={opt.id}>{opt.name}</SelectItem>
                            ))}
                          </SelectContent>
                        </Select>
                      )} />
                    </div>
                    <div>
                      <Label>Type *</Label>
                      <Controller name="maintenance_type" control={control} render={({ field }) => (
                        <Select onValueChange={field.onChange} value={field.value}>
                          <SelectTrigger className="mt-1"><SelectValue /></SelectTrigger>
                          <SelectContent>
                            <SelectItem value="routine">Routine</SelectItem>
                            <SelectItem value="repair">Repair</SelectItem>
                            <SelectItem value="replacement">Replacement</SelectItem>
                            <SelectItem value="inspection">Inspection</SelectItem>
                          </SelectContent>
                        </Select>
                      )} />
                    </div>
                    <div>
                      <Label>Cost</Label>
                      <Input className="mt-1" type="number" step="0.01" {...register('cost')} />
                    </div>
                    <div className="col-span-2">
                      <Label>Description *</Label>
                      <Textarea className="mt-1" rows={2} {...register('description')} />
                      {errors.description && <p className="text-xs text-red-500 mt-1">{errors.description.message}</p>}
                    </div>
                    <div>
                      <Label>Scheduled Date</Label>
                      <Input className="mt-1" type="date" {...register('scheduled_date')} />
                    </div>
                    <div>
                      <Label>Completed Date</Label>
                      <Input className="mt-1" type="date" {...register('completed_date')} />
                    </div>
                    <div>
                      <Label>Performed By</Label>
                      <Controller name="performed_by" control={control} render={({ field }) => (
                        <Select onValueChange={field.onChange} value={field.value ?? ''}>
                          <SelectTrigger className="mt-1"><SelectValue placeholder="Select user" /></SelectTrigger>
                          <SelectContent>{users.map(u => <SelectItem key={u.id} value={u.id}>{u.first_name} {u.last_name}</SelectItem>)}</SelectContent>
                        </Select>
                      )} />
                    </div>
                    <div className="col-span-2">
                      <Label>Notes</Label>
                      <Textarea className="mt-1" rows={2} {...register('notes')} />
                    </div>
                  </div>
                  <div className="flex justify-end gap-2 pt-2">
                    <Button type="button" variant="outline" onClick={() => { setDialogOpen(false); setEditRecord(null); reset(); }}>
                      Cancel
                    </Button>
                    <Button type="submit" className="bg-blue-600 hover:bg-blue-700" disabled={isSubmitting}>
                      {isSubmitting ? 'Saving...' : editRecord ? 'Update' : 'Add'}
                    </Button>
                  </div>
                </form>
              </DialogContent>
            </Dialog>
          </Can>
        </div>
      </PageHeader>

      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <KPICard title="Total Records" value={maintenance.length} icon={<Wrench className="h-4 w-4 text-blue-600" />} iconBg="bg-blue-50 dark:bg-blue-950/50" loading={loading} />
        <KPICard title="Scheduled" value={scheduled} icon={<Calendar className="h-4 w-4 text-orange-600" />} iconBg="bg-orange-50 dark:bg-orange-950/50" loading={loading} />
        <KPICard title="Completed" value={completed} icon={<Wrench className="h-4 w-4 text-emerald-600" />} iconBg="bg-emerald-50 dark:bg-emerald-950/50" loading={loading} />
        <KPICard title="Total Cost" value={`$${totalCost.toFixed(2)}`} icon={<DollarSign className="h-4 w-4 text-blue-600" />} iconBg="bg-blue-50 dark:bg-blue-950/50" loading={loading} />
      </div>

      <DataTable
        data={maintenance}
        columns={columns}
        loading={loading}
        searchPlaceholder="Search maintenance records..."
        searchKeys={['description', 'notes']}
        pageSize={15}
        emptyTitle="No maintenance records"
        emptyDescription="Add maintenance records to get started"
      />

      <ConfirmDialog
        open={!!deleteId}
        onClose={() => setDeleteId(null)}
        onConfirm={handleDelete}
        loading={deleting}
        title="Delete Maintenance Record?"
        description="This will permanently delete the maintenance record."
        confirmLabel="Delete"
      />
      </div>
    </PermissionGuard>
  );
}
