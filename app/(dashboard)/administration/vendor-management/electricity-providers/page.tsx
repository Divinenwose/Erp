...
import { Zap, Plus, Edit, Trash2, Phone, Mail, MapPin, DollarSign } from 'lucide-react';
...
export default function ElectricityProvidersPage() {
  ...
  useRealtimeTable('electricity_providers', company?.id, load);
  ...
  const onSubmit = async (data: ProviderForm) => {
    ...
    if (editProvider) {
      ...
      await logAuditEvent(company.id, currentUser?.id || '', {
        action: 'electricity_provider_updated',
      ...
      const { error } = await supabase.from('electricity_providers').insert({
      ...
      await logAuditEvent(company.id, currentUser?.id || '', {
        action: 'electricity_provider_created',
        module: 'vendor_management',
        new_values: { name: data.name },
      });
  ...
  const handleDelete = async () => {
    ...
    const { error } = await supabase.from('electricity_providers').delete().eq('id', deleteId);

    if (error) {
      toast.error('Failed to delete provider');
    } else {
      await logAuditEvent(company.id, currentUser?.id || '', {
        action: 'electricity_provider_deleted',
        module: 'vendor_management',
        record_id: deleteId,
      });
  ...
  const handleStatusUpdate = async (id: string, status: string) => {
    ...
    if (error) {
      ...
      await logAuditEvent(company.id, currentUser?.id || '', {
        action: 'electricity_provider_status_updated',
        module: 'vendor_management',
        record_id: id,
        new_values: { status },
      });
  ...
  const exportCSV = () => {
    const headers = ['Name', 'Contact Person', 'Email', 'Phone', 'Plan Type', 'Contract Start', 'Contract End', 'Monthly Cost', 'Status', 'Branch'];
    ...
    a.href = url;
    a.download = 'electricity_providers.csv';
    a.click();
  };
  ...
  const columns: Column<any>[] = [
    ...
    },
    {
      key: 'plan_type',
      header: 'Plan',
      cell: (row) => <span className="text-sm text-gray-600 dark:text-gray-400">{row.plan_type || '—'}</span>,
    },
    {
      key: 'contract_end',
      header: 'Contract End',
          ...
          <DropdownMenuContent align="end">
            ...
            {row.status === 'active' && (
              <DropdownMenuItem onClick={() => handleStatusUpdate(row.id, 'inactive')}>
                <Zap className="h-4 w-4 mr-2" />Deactivate
              </DropdownMenuItem>
            )}
            {row.status === 'inactive' && (
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="Manage electricity service providers"
        breadcrumbs={[{ label: 'Administration' }, { label: 'Vendor Management' }, { label: 'Electricity Providers' }]}
      >
        <div className="flex gap-2">
          ...
          <Can resource="vendor_management.electricity" action="create">
            <Dialog open={dialogOpen} onOpenChange={open => { if (!open) { setEditProvider(null); reset(); } setDialogOpen(open); }}>
              ...
              <DialogContent>
                <DialogHeader>
                  <DialogTitle>{editProvider ? 'Edit Provider' : 'Add Electricity Provider'}</DialogTitle>
                </DialogHeader>
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 pt-2">
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <Input className="mt-1" {...register('plan_type')} placeholder="e.g., Commercial, Industrial" />
                    </div>
                    <div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <KPICard title="Total Providers" value={providers.length} icon={<Zap className="h-4 w-4 text-blue-600" />} iconBg="bg-blue-50 dark:bg-blue-950/50" loading={loading} />
      ...
      <DataTable
        ...
        emptyDescription="Add electricity providers to get started"
      />
