...
import { Wrench, Plus, Edit, Trash2, Phone, Mail, MapPin, DollarSign } from 'lucide-react';
...
export default function MaintenanceVendorsPage() {
  ...
  useRealtimeTable('maintenance_vendors', company?.id, load);
  ...
  const onSubmit = async (data: VendorForm) => {
    ...
    if (editVendor) {
      ...
      await logAuditEvent(company.id, currentUser?.id || '', {
        action: 'maintenance_vendor_updated',
      ...
      const { error } = await supabase.from('maintenance_vendors').insert({
      ...
      await logAuditEvent(company.id, currentUser?.id || '', {
        action: 'maintenance_vendor_created',
        module: 'vendor_management',
        new_values: { name: data.name },
      });
  ...
  const handleDelete = async () => {
    ...
    const { error } = await supabase.from('maintenance_vendors').delete().eq('id', deleteId);

    if (error) {
      toast.error('Failed to delete vendor');
    } else {
      await logAuditEvent(company.id, currentUser?.id || '', {
        action: 'maintenance_vendor_deleted',
        module: 'vendor_management',
        record_id: deleteId,
      });
  ...
  const handleStatusUpdate = async (id: string, status: string) => {
    ...
    if (error) {
      ...
      await logAuditEvent(company.id, currentUser?.id || '', {
        action: 'maintenance_vendor_status_updated',
        module: 'vendor_management',
        record_id: id,
        new_values: { status },
      });
  ...
  const exportCSV = () => {
    ...
    a.href = url;
    a.download = 'maintenance_vendors.csv';
    a.click();
  };
          ...
          <DropdownMenuContent align="end">
            ...
            {row.status === 'active' && (
              <DropdownMenuItem onClick={() => handleStatusUpdate(row.id, 'inactive')}>
                <Wrench className="h-4 w-4 mr-2" />Deactivate
              </DropdownMenuItem>
            )}
            {row.status === 'inactive' && (
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="Manage facility maintenance service vendors"
        breadcrumbs={[{ label: 'Administration' }, { label: 'Vendor Management' }, { label: 'Maintenance Vendors' }]}
      >
        <div className="flex gap-2">
          ...
          <Can resource="vendors.maintenance" action="create">
            <Dialog open={dialogOpen} onOpenChange={open => { if (!open) { setEditVendor(null); reset(); } setDialogOpen(open); }}>
              ...
              <DialogContent>
                <DialogHeader>
                  <DialogTitle>{editVendor ? 'Edit Vendor' : 'Add Maintenance Vendor'}</DialogTitle>
                </DialogHeader>
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 pt-2">
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <Input className="mt-1" {...register('service_type')} placeholder="e.g., HVAC, Plumbing" />
                    </div>
                    <div className="col-span-2">
                      <Label>Address</Label>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <KPICard title="Total Vendors" value={vendors.length} icon={<Wrench className="h-4 w-4 text-blue-600" />} iconBg="bg-blue-50 dark:bg-blue-950/50" loading={loading} />
        <KPICard title="Active" value={active} icon={<Wrench className="h-4 w-4 text-emerald-600" />} iconBg="bg-emerald-50 dark:bg-emerald-950/50" loading={loading} />
      ...
      <DataTable
        ...
        emptyDescription="Add maintenance vendors to get started"
      />
