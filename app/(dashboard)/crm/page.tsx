...
export default function CRMOverviewPage() {
  const modules = [
    { title: 'Leads', description: 'Track potential customers', icon: Target, href: '/crm/leads', color: 'bg-blue-50 dark:bg-blue-950/30', iconColor: 'text-blue-600' },
    { title: 'Pipeline', description: 'Manage opportunities', icon: TrendingUp, href: '/crm/pipeline', color: 'bg-emerald-50 dark:bg-emerald-950/30', iconColor: 'text-emerald-600' },
    { title: 'Customers', description: 'Customer accounts', icon: Users, href: '/crm/customers', color: 'bg-violet-50 dark:bg-violet-950/30', iconColor: 'text-violet-600' },
    { title: 'Sales Orders', description: 'Order management', icon: ShoppingCart, href: '/crm/orders', color: 'bg-amber-50 dark:bg-amber-950/30', iconColor: 'text-amber-600' },
    { title: 'Contacts', description: 'Contact directory', icon: UserCheck, href: '/crm/contacts', color: 'bg-pink-50 dark:bg-pink-950/30', iconColor: 'text-pink-600' },
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <KPICard title="Pipeline Value" value={formatCurrency(totalPipelineValue)} change={15.3} changeLabel="vs last month" icon={<DollarSign className="h-4 w-4 text-emerald-600" />} iconBg="bg-emerald-50 dark:bg-emerald-950/50" />
        <KPICard title="Active Leads" value={65} change={8.5} changeLabel="vs last month" icon={<Target className="h-4 w-4 text-blue-600" />} iconBg="bg-blue-50 dark:bg-blue-950/50" />
        <KPICard title="Total Customers" value={48} change={4.3} changeLabel="vs last month" icon={<Users className="h-4 w-4 text-violet-600" />} iconBg="bg-violet-50 dark:bg-violet-950/50" />
        <KPICard title="Win Rate" value="38%" change={5.2} changeLabel="vs last month" icon={<Award className="h-4 w-4 text-amber-600" />} iconBg="bg-amber-50 dark:bg-amber-950/50" />
      </div>
